...
walkDir(srcDir, (filePath) => {
    ...
    content = content.replace(/bg-zinc-900 text-white/g, 'bg-[#a3e635] text-zinc-900 font-bold');
    ...
    content = content.replace(/bg-indigo-50/g, 'bg-[#f4fce3]');
    content = content.replace(/text-indigo-700/g, 'text-[#3f6212]');
    content = content.replace(/border-indigo-100/g, 'border-[#ecfccb]');
    ...
    // Explicit exception for LandingPage and Sidebar primary CTAs if any overlap occurred that shouldn't have:
    // Actually the CTA buttons will perfectly update!
